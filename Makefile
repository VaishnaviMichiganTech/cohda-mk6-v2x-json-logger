...
#############################################################################
# Enable support for TIMs
#############################################################################

EXTRA_CFLAGS += -DWITH_TIMRX_CSW -DWITH_TIMRX_GENERIC
...
ifneq ($(NO_VIICSEC),1)
  ...
  SEC_LDLIBS  ?= -ldl -lviicsec -laerolinkPKI -lssl -lcrypto
...
ifneq (,$(findstring $(BOARD),mk5))
  ...
  SEC_V2XLIBS := libviicsec.so libviicsec.so.1 libaerolinkPKI.so libaerolinkPKI.so.1 \
                 ...
                 libNxDispatcher.so libNxDispatcher.so.1.9 libNxDispatcher.so.1.9.3
  SEC_LDLIBS  := -lviicsec -lsxa1700Interface -laerolinkPKI -lssl -lcrypto -lc2x -lNxDispatcher -ldl
  ...
  QTI_LDLIBS := -L $(BSP_DIR)/QTI/lib64 \
                 ...
                 -ldiag -lqcmapipc -lqdi -lnetmgr -lrmnetctl -lconfigdb -lxml -lgthread-2.0 -lglib-2.0 -lpcre -lprotobuf-nanopb -lprotobuf -lprotobuf-lite -lcap -lz
...
AEROLINK_TOOLS =  \
		 ...
		 $(OUTDIR)/aerolink/createECRequest \
		 $(OUTDIR)/aerolink/processECResponse
ifneq (,$(findstring $(BOARD),mk5 mk6 mk6c x86_64))
AEROLINK_TOOLS +=  \
		 $(OUTDIR)/aerolink/canonicalDataReader
endif
# currently only x86_64 and MK6 have these tools
# (they are needed for 1609.2.1)
ifneq (,$(findstring $(BOARD),mk6 mk6c x86_64))
# Ubuntu 18.04 uses an older version of the Aerolink library that does
# not contain these tools
OS_VERS := $(shell sed --regexp-extended --quiet --expression 's/VERSION_ID="(.*)"/\1/p' /etc/os-release )
ifneq (x86_64__18.04,$(BOARD)__$(OS_VERS))
AEROLINK_TOOLS +=  \
		 $(OUTDIR)/aerolink/createIeeeEndEntityRegistration \
		 $(OUTDIR)/aerolink/processIeeeEndEntityRegistration
endif
endif
endif
...
PCAP_INC_DIR ?= $(realpath $(CURDIR)/../../../bsp/app/libpcap/$(BOARD))
ifneq (,$(PCAP_INC_DIR))
  CFLAGS += -I$(PCAP_INC_DIR)
endif
ifneq (,$(PCAP_LIB_DIR))
...
ifneq (,$(LIBNL_LIB_DIR))
  LDFLAGS += -L$(LIBNL_LIB_DIR) -L$(LIBNL_LIB_DIR)/lib/.libs
endif
...
ifeq ($(POSIX),1)
  ...
  V2XLIBS += lib1609.a libnl-3.so libnl-3.so.200 libnl-3.so.200.24.0 libnl-genl-3.so libnl-genl-3.so.200 libnl-genl-3.so.200.24.0
  LDLIBS += -l1609 -lnl-3 -lnl-genl-3
endif
...
NTRIP_SRCS = $(wildcard $(SRCDIR)/src/Ntrip/*.c)
NTRIP_OBJS = $(subst $(SRCDIR),$(OUTDIR),$(NTRIP_SRCS:.c=.o))
NTRIP_LIB  = $(OUTDIR)/lib/libNtrip.so
...
DEPS = $(OBJS:.o=.d) $(NTRIP_OBJS:.o=.d) $(RLW_OJGS:.o=.d)
...
$(eval $(call GenerateRules, $(OUTDIR)/src/Ntrip))
$(eval $(call GenerateRules, $(OUTDIR)/src/RLW))

# Include dependencies
-include $(DEPS)

src-gen: asn-gen ntrip-gen rlw-gen ## Create all generated code
.PHONY: src-gen

app: version src-gen board/board.conf $(OUTDIR) ## Build $(APP) binary (Append BOARD=? to specify a non-host target)
...
$(STACKDIR)/v2x-lib/include $(STACKDIR)/v2x-lib/lib/$(BOARD):
	...
	if [ -e $(STACKDIR)/Makefile ]; then \
	  $(MAKE) -C $(STACKDIR) BOARD=$(BOARD) lib-package POSIX=$(POSIX) VARIANT=$(VARIANT) ; \
	fi;
...
ntrip: $(NTRIP_LIB) ## Build the 'NTRIP client' plugin

$(SRCDIR)/src/Ntrip/Ntrip_debug.h: $(SRCDIR)/src/Ntrip/Ntrip.conf
	cd $(@D) && \
	  if [ -e $(STACKDIR)/tools/conf ] ; then  \
	    $(STACKDIR)/tools/conf    --conf $< --prefix Cust_App_Ntrip --compact --no-cpp; \
	  else \
	    $(STACKDIR)/tools/conf.py --conf $< --prefix Cust_App_Ntrip --compact --no-cpp; \
	  fi ;

ntrip-gen: $(SRCDIR)/src/Ntrip/Ntrip_debug.h

.PHONY: ntrip ntrip-gen

$(NTRIP_LIB): $(OUTDIR)/include $(V2XLIBS) $(NTRIP_OBJS)
ifneq (,$(findstring $(BOARD),mq5))
	$(CC) -shared $(LDFLAGS) $(LDLIBS) -Wl,--no-keep-memory -o $@ $(NTRIP_OBJS)
else
	$(CC) -shared $(LDFLAGS)                                -o $@ $(NTRIP_OBJS)
endif


rlw: $(RLW_LIB) ## Build the 'RLW client' plugin
...
$(SRCDIR)/src/Example1609_debug.h: $(SRCDIR)/src/Example1609.conf
	cd $(@D) && \
	if [ -e $(STACKDIR)/tools/conf ] ; then  \
	  $(STACKDIR)/tools/conf    --conf $< --prefix Example1609 --compact --no-cpp; \
	else \
	  $(STACKDIR)/tools/conf.py --conf $< --prefix Example1609 --compact --no-cpp; \
	fi ;

src-gen: $(SRCDIR)/src/Example1609_debug.h

# Build the application
$(OUTDIR)/$(APP): $(OUTDIR)/include $(V2XLIBS) $(NTRIP_LIB) $(RLW_LIB) $(OBJS) $(LIBS)
ifneq (,$(findstring $(BOARD),mq5))
...
clean: ## Clean up. Use `BOARD`-clean to specify the target (e.g. mk5-clean)
	@# Generated source(s)
	rm -rf $(SRCDIR)/src/Ntrip/Ntrip_*
	rm -rf $(SRCDIR)/src/RLW/RLW_*
	rm -rf $(SRCDIR)/src/Example1609_*
	@# Build tree
	$(SUDO) rm -rf $(OUTDIR)
